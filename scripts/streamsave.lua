...
local function write_chapter(chapter)
	...
	if chapter_list[chapter] or zeroth_chapter then
		segments[1] = {
			["start"] = zeroth_chapter and cache_start or chapter_list[chapter]["time"],
			["end"] = chapter_list[chapter + 1] and chapter_list[chapter + 1]["time"] or "no",
			["title"] = chapter .. ". " .. (not zeroth_chapter and chapter_list[chapter]["title"] or file.title),
		}
...
function cache_write(mode, quiet, chapter)
	...
	if file.pending == 2 or segments[1] and file.pending > 0 and not loop.continuous then
	...
	if sid and not subs["external"] then
		if
			file.ext == ".mp4"
			or subs["codec"] == "webvtt-webm"
			or file.ext == ".webm" and subs["codec"] ~= "webvtt"
		then
			msg.warn(
...
function get_chapters()
	...
	if not current_chapters[1] or not string.match(current_chapters[1]["title"], "^[AB] loop point$") then
...
function get_seekable_cache(prop, range_check)
	...
	local seekable_ranges, cache_state = get_ranges()
	if prop then
		if range_check ~= false and (#seekable_ranges == 0 or not cache_state["cache-end"]) then
			reset()
...
local function seamless(_, cache_state)
	...
	if reader >= cache.seekend - 0.25 or cache.prior - cache_duration > 3000 or cache_state["underrun"] then
		reload()
...
function automatic(_, cache_time)
	...
	if opts.autostart and not cache.dumped and (not cache.endsec or cache_time < cache.endsec or opts.piecewise) then
	...
	-- stop cache dump
	if cache.endsec and cache.dumped and cache_time - cache.part >= cache.endsec then
...
function autoquit()
	...
	elseif not file.quit_timer then
		file.quit_timer = mp.add_timeout(file.quitsec, function()
			stop()
			mp.command("quit")
	...
	if t.prefix == "ffmpeg/demuxer" then
		...
		if packets then
			...
			if cache_time then
				...
				cache.packets[stamp] = mp.add_periodic_timer(3, function()
...
mp.register_script_message("streamsave-chapter", function(chapter)
	cache_write("chapter", _, tonumber(chapter))
end)

mp.add_key_binding("Alt+z", "mode-switch", function()
	mode_switch("cycle")
end)
mp.add_key_binding("Ctrl+x", "stop-cache-write", stop)
mp.add_key_binding("Alt+x", "align-cache", align_cache)
mp.add_key_binding("Ctrl+z", "cache-write", function()
	cache_write(opts.dump_mode)
end)
