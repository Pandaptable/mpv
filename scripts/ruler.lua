...
function refresh()
	if not video_dimensions_stale then
		return
	end
	...
	local distinct = (
		math.abs(line_start.screen[1] - line_end.screen[1]) >= 1
	...
	local line_info = function()
		if not opts.show_distance then
			return
		end
		a:new_event()
		a:append("{\\fs36}{\\bord1}")
		...
		local an = 1
		if line_start.image[1] < line_end.image[1] then
			an = an + 2
		end
		if line_start.image[2] < line_end.image[2] then
			an = an + 6
		end
		a:an(an)
		local image = math.sqrt(
			math.pow(line_start.image[1] - line_end.image[1], 2) + math.pow(line_start.image[2] - line_end.image[2], 2)
		)
		local screen = math.sqrt(
			math.pow(line_start.screen[1] - line_end.screen[1], 2)
				+ math.pow(line_start.screen[2] - line_end.screen[2], 2)
		)
	...
	local dot_info = function(pos, opposite)
		if not opts.show_coordinates then
			return
		end
		a:new_event()
	...
	if distinct and opts.show_angles ~= "no" then
		...
		local angle = math.atan(
			math.abs(line_start.image[2] - line_end.image[2]) / math.abs(line_start.image[1] - line_end.image[1])
		)
		local fix_angle
		local an
