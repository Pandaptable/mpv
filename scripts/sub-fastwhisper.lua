...
local o = {
	...
	fast_whisper_path = "D:\\PATH\\Faster-Whisper-XXL\\faster-whisper-xxl.exe",
...
local function get_current_sub()
	...
	if sub then
		if
			sub.external
			and not is_protocol(sub["external-filename"])
			and sub["external-filename"]:lower():match("%.srt$")
		then
...
local function generate_ass(subtitles_file)
	...
	file:write(
		"Style: Default,"
			.. o.font_name
			.. ",70,&H00FFFFFF,&HF0000000,&H00000000,&H32000000,0,0,0,0,100,100,0,0.00,1,2,1,2,5,5,2,-1\n"
	)
	file:write("\n")
	file:write("[Events]\n")
...
local function fastwhisper_cache(current_pos, subtitle_count)
	...
	if not valid_range or cache_end <= current_pos then
		mp.add_timeout(1, function()
			fastwhisper_cache(current_pos, subtitle_count)
		end)
		return
	end
